...
class Simulator(object):
	...
	def plot(self, t):
		...
		#plt.show()
		filename = 'Plots/Renting/small_world/' + str(t) + '.png'
		plt.savefig(filename, format="PNG")
		plt.clf()
...
# iterate over graph_args and initial_prefs:
# 	not stubborn, no budgets, heterogenous

initial_proportions = [[0.5, 0.5, 0.], [.25, .75, 0.], [.4, .6, 0.], [.1, .9, 0.]] #, [0.25, 0.75, 0.], [0.4, 0.6, 0.]]
stubborn_proportions = [.2, .4, .5, .75, .9] 
# prices = [{0 : 1, 1 : 9}, {0 : 3, 1 : 7}, {0 : 5, 1 : 5}]

network = 'small_world'
#args = [{'k' : 2, 'p' : .1,}, {'k' : 5, 'p' : .1}, {'k' : 10, 'p' : .1}, {'k' : 2, 'p' : .5}, {'k' : 5, 'p' : .5}, {'k' : 10, 'p' : .5}]
agent_arg = {'k':0., 'n':1., 'heterogeneous': False, 'stubborn_prop': 0.0}
arg = {'k' : 5, 'p' : .1}
for proportions in initial_proportions:
	test_sim = Simulator(proportions, network_structure=network, graph_args=arg, agent_args=agent_arg, num_iters=100, budget=2)
	test_sim.simulate()
	#test_sim.simulate_n(100)
		# for stubborn in stubborn_proportions:
	# for price in prices:
	# 	arg['prices'] = price
			# test_sim = Simulator(proportions, network_structure=network, graph_args=arg, agent_args={'k':0., 'n':1., 'heterogeneous': False, 'stubborn_prop': stubborn}, num_iters=500, budget=False)
		#test_sim.simulate_n(100)
		# test_sim.simulate()
		# test_sim = Simulator(proportions, network_structure=network, graph_args=arg, agent_args={'heterogeneous': False, 'k':0, 'n':1, 'stubborn_prop':0.}, num_iters=500, budget=False)
			# test_sim.simulate_n(100)
		# test_sim.simulate(plot_stats=True)
'''
network = 'erdos_renyi'
#args = [{'p': .1}, {'p': .15}, {'p': .25}, {'p': .5}]
arg = {'p': .2}
for proportions in initial_proportions:
		#test_sim = Simulator(proportions, network_structure=network, graph_args=graph_arg, agent_args=agent_arg, num_iters=100, budget=False)
		#test_sim.simulate_n(100)
	# 	for stubborn in stubborn_proportions:
	# # for price in prices:
	# # 	arg['prices'] = price
...
#args = [{'m': 1}, {'m': 2}, {'m': 5}, {'m': 10}]
arg = {'m': 5}
for proportions in initial_proportions:
	#for arg in args:
		#test_sim = Simulator(proportions, network_structure=network, graph_args=graph_arg, agent_args=agent_arg, num_iters=200, budget=False)
		#test_sim.simulate_n(100)
		# 	for stubborn in stubborn_proportions:
		# # for price in prices:
	#arg['prices'] = {0 : 1, 1: 1, 2:0}  #price
	test_sim = Simulator(proportions, network_structure=network, graph_args=arg, agent_args=agent_arg, num_iters=100, budget=1)
	#test_sim.simulate()
	test_sim.simulate_n(100)

proportions = [.5, .5]
args = {'k' : 2, 'p' : .1, 'prices' : {0 : 5, 1 : 5}}
test_sim = Simulator(proportions, network_structure='small_world', graph_args=args, agent_args={'k':0., 'n':1., 'heterogeneous': False, 'stubborn_prop': 0.0}, num_iters=100, budget=True)
test_sim.plot()
'''
